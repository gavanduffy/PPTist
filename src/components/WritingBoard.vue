...
// Mouse Position Coordinates：Use to draw pens or rubber positions to follow
const mouse = ref({
  x: 0,
  y: 0,
})

// Whether the mouse is within the canvas：You can only show brushes or rubbers in the range.
const mouseInCanvas = ref(false)

// Listen to UpdatescanvasDimensions
...
// Toggle brush mode，Update canvas ctx Configure
const updateCtx = () => {
  if (!ctx) return
  if (props.model === 'mark') {
...
// Calculate the distance between two moves of the mouse
...
// Move distance between two moves by mousesAnd time.tCalculate drawing speed，The faster the speed.，The thinner the ink.
...
// Get Mouse IncanvasRelative position in
...
// Prepare to begin drawing./Wipe Ink（Do it.）
...
// Settings DataURL（Draw pictures to canvas）
