...
export default () => {
  ...
  const uniformHorizontalDisplay = () => {
    ...
    // Collect individual elements and grouped elements while recording their ranges
    ...
    // Merge the individual and grouped elements, then sort them from left to right
    const list: Item[] = [...singleElemetList, ...formatedGroupList]
    list.sort((itemA, itemB) => itemA.min - itemB.min)

    // Compute the spacing for uniform distribution:
    // (total range of selected elements - sum of widths) / (count - 1)
    let totalWidth = 0
    ...
    // Calculate the target position for each item in order.
    // The first item acts as the baseline. Each subsequent position equals the
    // previous position plus its width and the spacing. Note that the computed
    // position is the minimum bound, not the final left value for rotated
    // elements.
    ...
    // Derive each element's final left value from the target position.
    // For rotated elements, correct by accounting for the rotation offset.
  ...
  // Distribute elements evenly along the vertical axis (mirrors horizontal logic)
