...
export default () => {
  ...
  // Export a pptist file (custom .pptist extension)
  ...
  // Format color values as opacity + hex string for pptxgenjs
  ...
  // Format HTML strings to match pptxgenjs expectations.
  // Core idea: flatten the HTML into styled slices, inheriting ancestor styles
  // and inserting line breaks for block-level elements.
  ...
  // Convert SVG path data into the structure required by pptxgenjs
  ...
  // Determine whether the url is a Base64 image
  const isBase64Image = (url: string) => {
    const regex = /^data:image\/[^;]+;base64,/
    return url.match(regex) !== null
  }

  // Determine whether the url references an SVG image
      ...
      for (const el of slide.elements) {
        if (el.type === 'text') {
          ...
          if (el.special) {
            ...
            if (svgRef.clientWidth < 1 || svgRef.clientHeight < 1) continue // Temporary guard for malformed PPTX imports
