...
// Various interactive event monitoring for rich text：
// Cancel global shortcut key event when focused
// Synchronize data when entering textvuex
// Synchronize rich text status to toolbar when clicking mouse and keyboard
...
const handleFocus = () => {
  // Multiple selection and pressedctrlorshiftKey time，Do not disable global shortcut keys
...
// Open/Close the editor's editing mode
...
// Some commands will first check whether the current selection is empty before execution.，If the selection is empty, select all first.
  ...
  for (const item of actions) {
    if (item.command === 'fontname' && item.value !== undefined) {
      ...
      if (item.value && !document.fonts.check(`16px ${item.value}`)) {
        message.warning('The font needs to wait for loading and downloading before it takes effect.，please wait')
      }
    }
...
// When the mouse is raised，Execute format brush command
const handleMouseup = () => {
...
// ProsemirrorEditor initialization and uninstallation
onMounted(() => {
