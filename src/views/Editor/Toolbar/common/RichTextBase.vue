  ...
  <div class="rich-text-base">
    ...
    <ButtonGroup class="row" passive>
      <Popover trigger="click" v-model:value="AIPopoverVisible" style="width: 25%;">
        <template #content>
          <PopoverMenuItem center @click="execAI('Beautify and rewrite')">beautify</PopoverMenuItem>
          <PopoverMenuItem center @click="execAI('Rich expansion')">Expand</PopoverMenuItem>
          <PopoverMenuItem center @click="execAI('Streamline and refine')">streamline</PopoverMenuItem>
        </template>
        <CheckboxButton
          first
          style="width: 100%;"
          v-tooltip="'AIAuxiliary'"
    ...
    <div class="row">
      <ButtonGroup style="flex: 1;" passive>
        <Button first style="flex: 1;" v-tooltip="'Reduce paragraph indent'" @click="emitRichTextCommand('indent', '-1')"><IconIndentLeft /></Button>
        <Popover trigger="click" v-model:value="indentLeftPanelVisible">
          <template #content>
            <PopoverMenuItem center @click="emitRichTextCommand('textIndent', '-1')">Reduce first line indent</PopoverMenuItem>
          </template>
          <Button last class="popover-btn"><IconDown /></Button>
        </Popover>
      </ButtonGroup>
...
const updateLink = (link?: string) => {
  ...
  if (!link || !linkRegExp.test(link)) return message.error('Incorrect web link address')
...
const execAI = async (command: string) => {
  ...
  if (!content) return message.error('No executable text content')
