<template>
  <div class="image-style-panel">
    ...
    <ButtonGroup class="row" passive>
      <Button first style="width: calc(100% / 6 * 5);" @click="clipImage()"><IconTailoring /> Crop picture</Button>
        ...
        <template #content>
          <div class="clip">
            ...
            <template v-for="typeItem in ratioClipOptions" :key="typeItem.label">
              <div class="title" v-if="typeItem.label">according to{{typeItem.label}}：</div>
    ...
    <FileInput @change="files => replaceImage(files)">
      <Button class="full-width-btn"><IconTransform /> Replace picture</Button>
    </FileInput>
    <Button class="full-width-btn" @click="resetImage()"><IconUndo /> reset style</Button>
    <Button class="full-width-btn" @click="setBackgroundImage()"><IconTheme /> set as background</Button>
  </div>
</template>
...
const getImageElementDataBeforeClip = () => {
  ...
  // The current position, size and cropping range of the image
...
const presetImageClip = (shape: string, ratio = 0) => {
  ...
  // Aspect ratio cropping（Shape is fixed to rectangular）
  if (ratio) {
  ...
  // Shape cropping（Keep current cropping range）
  else {
...
// Replace picture（Keep current style）
...
// Reset picture：Clear all styles
const resetImage = () => {
  const _handleElement = handleElement.value as PPTImageElement
