    ...
    <div class="btns">
      <div class="left">
        ...
        <Popover trigger="click" placement="top" v-model:value="chartTypeSelectVisible">
          ...
          <span class="change">Click to replace</span>
        </Popover>
      </div>
      <div class="right">
...
// The position of the border line of the current selection
...
// Initialize chart data：Format and populate the data intoDOM
const initData = () => {
...
// Shortcut key monitoring：Enter moves focus to the next line
const moveNextRow = () => {
...
// Get the current chartDOMdata in，Organize and format and send it out
const getTableData = () => {
  ...
  // The first line is the series name，The first column is the project name，The actual data starts from the second row and second column
  ...
  // Some special chart types，Data needs to be processed separately
  // The scatter plot has and only two columns of data
...
// Custom paste event（Try to read table data from clipboard）
...
// Drag the mouse to modify the selected data range
const changeSelectRange = (e: MouseEvent) => {
  ...
  document.onmouseup = e => {
    ...
    // When dragging ends，If the range exceeds half of the grid, it will automatically expand to the next grid.（For example, drag to a position more than one and a half squares，will automatically expand to two spaces，The same applies regardless）
