      ...
      <div class="notes" ref="notesRef">
        <div class="note" :class="{ 'active': activeNoteId === note.id }" v-for="note in notes" :key="note.id" @click="handleClickNote(note)">
          ...
          <div class="note-reply" v-if="replyNoteId === note.id">
            <TextArea :padding="6" v-model:value="replyContent" placeholder="Enter reply" :rows="1" @enter.prevent="createNoteReply()" />
        ...
        <div class="empty" v-if="!notes.length">No notes on this slide yet</div>
      </div>
      <div class="send">
        <TextArea
          ref="textAreaRef"
          ...
          :padding="6"
          :placeholder="`Enter a note (for ${handleElementId ? 'selected element' : 'current slide' })`"
          :rows="2"
          @focus="replyNoteId = ''; activeNoteId = ''"
          @enter.prevent="createNote()"
        />
        <div class="footer">
          <IconDelete class="btn icon" v-tooltip="'Clear notes on this slide'" style="flex: 1" @click="clear()" />
          <Button type="primary" class="btn" style="flex: 12" @click="createNote()"><IconPlus /> Add Note</Button>
        </div>
      </div>
    </div>
  </MoveablePanel>
