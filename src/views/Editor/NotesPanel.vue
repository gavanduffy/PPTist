  ...
  >
    <div class="container">
      <div class="notes" ref="notesRef">
        ...
        <div class="empty" v-if="!notes.length">No notes on this page</div>
      </div>
      <div class="send">
        <TextArea 
          ref="textAreaRef"
          ...
          :padding="6"
          :placeholder="`Add note (for ${handleElementId ? 'selected element' : 'current slide' })`"
          :rows="2"
          @focus="replyNoteId = ''; activeNoteId = ''"
          @enter.prevent="createNote()"
        />
        <div class="footer">
          <IconDelete class="btn icon" v-tooltip="'Clear all notes on this page'" style="flex: 1" @click="clear()" />
