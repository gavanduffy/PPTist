<template>
  ...
  >
    <div class="add-slide">
      <div class="btn" @click="createSlide()"><IconPlus class="icon" />Add slides</div>
...
watch(() => slideIndex.value, () => {
  ...
  // Check whether the current page thumbnail is within the visible range，If it is not there, you need to scroll to the corresponding position.
  nextTick(() => {
...
const handleClickSlideThumbnail = (e: MouseEvent, index: number) => {
  ...
  // Press and holdCtrlkey，Click on the slide，Click the selected page again to deselect it.
  // If the unselected page happens to be the currently active page，You need to select the first one from other selected pages as the current active page.
  ...
  // Press and holdShiftkey，Select all slides in range
...
// Set thumbnail toolbar focus state（Only in focus state，Only the shortcut keys for this part can take effect）
...
// Drag and drop to adjust the order and then synchronize the data.
...
const contextmenusSection = (el: HTMLElement): ContextmenuItem[] => {
  ...
  return [
    ...
    {
      text: 'Delete sections and slides',
      handler: () => {
        mainStore.setActiveElementIdList([])
